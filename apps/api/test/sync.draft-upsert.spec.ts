...
async function ownerCtx(app: INestApplication, suffix: string) {
  const email = `up_${suffix}@example.com`;
...
describe('Sync draft upsert (T017)', () => {
  ...
  it('creates then returns same id on replay', async () => {
    if (!dbAvailable) return;
    const ctx = await ownerCtx(app, String(Date.now()));
    const key = `draft-key-${Date.now()}-abcd`;
    const body = draftBody(ctx.branchId, `INV-UP-${Date.now()}`);
    ...
    expect(created.body.clientIdempotencyKey).toBe(key);
    expect(created.body.syncRevision).toBe(1);
