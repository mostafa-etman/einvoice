...
describe('EtaDocumentsSearchClient direction filter', () => {
  ...
  it('uses issueDateFrom/To when dateField is issue (VAT-period sync)', async () => {
    let requested = '';
    const fetchImpl = (async (url: string | URL) => {
      requested = String(url);
      return new Response(JSON.stringify({ result: [] }), { status: 200 });
    ...
    expect(requested).toContain('issueDateFrom=');
    expect(requested).toContain('issueDateTo=');
    expect(requested).not.toContain('submissionDateFrom=');
  });
